...
| extraArgs.defaultSslCertificate | Sets the TLS certificate. {{< callout type=“warning >}}``extraArgs.defaultSslCertificate`` is managed by MKE and must not be changed manually. If you want to change the TLS certificates used by MKE 4k, refer to [TLS certificates](../tlscertificates). {{< /callout >}}| `mke/mke-ingress.tls` | 
